...
// Helper: shorten an address to just the primary city/area name (1 part only)
function shortenAddress(addr) {
  if (!addr) return "";
  const parts = addr.split(",").map(p => p.trim()).filter(Boolean);
  if (parts.length === 0) return addr;
  // Skip parts that start with numbers (street numbers / postal codes)
  const meaningfulParts = parts.filter(p => !/^\d/.test(p) && p.length > 2);
  // Return ONLY the first meaningful part to keep route column tight
  const best = meaningfulParts[0] || parts[0];
  // Truncate to 22 chars max to guarantee it fits
  return best.length > 22 ? best.slice(0, 20) + "…" : best;
}
...
export function generatePDF(quote, includeTolls = true) {
  return new Promise((resolve) => {
    ...
    const render = () => {
      ...
      const showTollsInPDF = hasTolls && includeTolls;
